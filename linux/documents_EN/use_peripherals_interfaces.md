...
⑦PL_UART\
⑧PS_UART\
⑨EMMC(Not visible in the image)\
⑩EEPROM(Not visible in the image)\
⑪QSPI FLASH(Not visible in the image)\
⑫DS1308(Not visible in the image)\
⑬LM75(Not visible in the image)\
...
---
### ①LEDS、KEYS
#### 1.1LEDs and KEYs on the development board
In addition to the POWER light and the RESET button, there are two sets of LEDs and KEY on the development board, namely, the LED and KEY on the PS side and the LED and KEY on the PL side :\
![](../.images_for_documents/48.png)\
![](../.images_for_documents/48_1.png)
#### 1.2How to use LEDs and KEYs in Linux
Using LEDs and KEYs is actually configuring and using GPIO. In Linux, you only need to know the GPIO number to use the corresponding IO port. The GPIO number of PS_LED is 377, the GPIO number of PS_KEY is 373, the GPIO number of PL_LED is 507, and the GPIO number of PL_KEY is 510.\
The IO corresponding to an LED needs to be configured for output. Taking a PS_LED as an example, the steps to use GPIO output are as follows :
```
...
#Make this port output high level to turn on the LED
echo 1 >> /sys/class/gpio/gpio377/value
#Make this port output low level to turn off the LED
...
#### 1.3Use the built-in scripts to test LEDs and KEYs
Run the *~/shells_for_testing_peripheral_devices/* script in *leds_keys_test.sh* (*ps: Press **ctrl+c** to exit the script.*). Then you can use PS_KEY to control the turning on and off of the PS_LED, and PL_KEY to control the turning on and off of the PS_LED :\
...
#### 2.2Use M.2 interface in the system
...
#### 2.3Use the built-in script in Linux to test M.2 SSD
...
#### 5.3Use the built-in script in Linux to test ETH
...
#### 6.3Use the built-in script in Linux to test USB Drive
...
#### 7.3Use the built-in script in Linux to test PL_UART
...
#### 9.3Use the built-in script in Linux to test EMMC
...
#### 11.2Using QSPI FLASH on Linux systems
...
```
#### 11.3Use the built-in script in Linux to test QSPI FLASH
...
#### 12.2Using DS1308 on Linux systems
...
#### 12.3Use the built-in script in Linux to test DS1308
...
#### 13.2Using LM75 on Linux systems
